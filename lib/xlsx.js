...
// 기본값: D열 이미지 · E열 코드 · 4행 시작 (Photo_Val.py 와 동일). 설정에서 변경 가능.
const DEFAULT_IMG_COL = 3;   // D (0-based)
const DEFAULT_CODE_COL = 4;  // E (0-based)
const DEFAULT_START_ROW = 4;

/** 열 문자("A","D","AB"…) → 0-based 인덱스 */
 ...
 */
async function readSheetImages(zip, sheetPath, sheetDoc, imgCol) {
  ...
  for (const anchor of anchors) {
    ...
    if (col !== imgCol) continue;                  // 지정한 이미지 열만
...
/** 시트에서 코드 열(행 → 문자열) 맵을 만든다. */
function readSheetCodes(sheetDoc, sharedStrings, codeCol) {
  const codes = {}; // { rowNumber: string }
  for (const c of sheetDoc.getElementsByTagName('c')) {
    const ref = c.getAttribute('r');
    if (!ref) continue;
    const pos = parseCellRef(ref);
    if (!pos || pos.col !== codeCol) continue;
 ...
 * @param {File} file
 * @param {{imgCol?:string|number, codeCol?:string|number, startRow?:number}} opts
 */
export async function parseWorkbook(file, opts = {}) {
  const imgCol = opts.imgCol == null ? DEFAULT_IMG_COL
    : (typeof opts.imgCol === 'number' ? opts.imgCol : colLetterToIndex(opts.imgCol));
  const codeCol = opts.codeCol == null ? DEFAULT_CODE_COL
    : (typeof opts.codeCol === 'number' ? opts.codeCol : colLetterToIndex(opts.codeCol));
  const startRow = opts.startRow > 0 ? opts.startRow : DEFAULT_START_ROW;
  ...
  for (const sheet of sheets) {
    ...
    const codes = readSheetCodes(sheetDoc, sharedStrings, codeCol);
    const images = await readSheetImages(zip, sheet.path, sheetDoc, imgCol);
