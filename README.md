...
3. **자동 매칭 (AI + 특징점 결합)** — 두 신호를 함께 씁니다.
   ...
   - **성공한 매칭이 자동으로 상단에 정렬**됩니다(그룹 내에서는 행 순서 유지).
   - **후보 변경**을 누르면 **큰 팝업**에서 엑셀 사진(300px)과 후보 사진들을 크게 비교하며 클릭 한 번으로 확정합니다(확정 즉시 닫힘).
   - 매칭이 끝나면 **"매칭 안 된 부품 코드"** 목록이 정리되어, **📋 복사** 또는 **⬇ 엑셀(CSV) 다운로드**로 바로 활용할 수 있습니다(코드·엑셀파일·시트·행).
   - AI 결과가 아쉬우면 **[🔍 ORB 정밀 재분석]** 버튼으로 다운로드·파싱 없이 ORB 검증만 한 번 더 돌릴 수 있습니다(애매한 저해상도 건을 추가로 잡아줌).
   - 진행 상황은 **% 진행바**로, 모델 최초 다운로드는 **% 오버레이**로 보여줍니다.
4. **배경 제거 · 내보내기** — 확정된 사진을 AI로 배경 제거 → 흰 배경 합성 → `{서비스코드}.jpg` 생성. 완료 후 두 가지를 따로 내려받을 수 있습니다:
   - **📦 사진 ZIP** — `엑셀파일명/코드.jpg` 구조의 ZIP
   - **📄 수정 엑셀** — **원본 부품리스트를 그대로 유지**한 채, 매칭 실패 행에 **"매칭 실패"(빨간 배경)** 를 기입한 `{원본명}_수정.xlsx`. 기입할 열(기본 A)은 ⚙ 설정 "매칭 결과 열"에서 지정
...
└─ lib/
   ...
   ├─ xlsx-writer.js  # 원본 엑셀 수정 (지정 열에 "매칭 실패" 표시)
